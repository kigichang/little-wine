...
fn main() -> Result<()> {
    ...
    let data: Vec<String> = lst
        .iter()
        .map(|wine| {
            // println!("{}", wine.profile);
            format!("{}", wine.profile)
        })
        .collect::<Vec<String>>();
    // println!("data:{:?}", data);
    // {
    //     let a = std::fs::File::create("data.json")?;
    //     serde_json::to_writer(a, &data)?;
    // }
    ...
    let top_k = similiarity
        ...
        .take(5)
        .map(|(i, _s)| {
            // println!("score: {}, idx: {}, name: {}", i, s, lst[*i].profile);
            lst[*i].profile.clone()
        })
        .collect::<Vec<_>>();
    ...
    let prompt = format!("你是位葡萄酒愛好者，我會提供酒友的問題，以及推薦的葡萄酒資料。請你依推薦的葡萄酒資料，撰寫一段評論。問題是：「{}」。以下是推薦的葡萄酒的 JSON 資料：{}", question, String::from_utf8_lossy(&buf).to_owned());
    println!("問:{}", question);
    //println!("{}", prompt);
